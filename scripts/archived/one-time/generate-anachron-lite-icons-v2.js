/**
 * Generate ANACHRON Lite Service Icons - V2
 * Using existing AI Orchestrator with proper OpenAI configuration
 */
...
/**
 * Generate a single service icon using DALL-E 3 (as gpt-image-1 doesn't exist yet)
 */
async function generateIcon(service, index, total) {
  ...
  try {
    // Use DALL-E 3 which is the actual available model
    ...
    console.log(`Generated image URL: ${imageUrl.substring(0, 80)}...`);
...
async function generateAllIcons() {
  console.log('=================================================');
  console.log('ANACHRON Lite Service Icons Generation - V2');
  console.log('=================================================');
  ...
  for (let i = 0; i < services.length; i++) {
    ...
    // Wait 65 seconds between generations to respect DALL-E 3 rate limits (60s + buffer)
    if (i < services.length - 1 && result.success) {
      console.log('\nWaiting 65 seconds before next generation (DALL-E 3 rate limit)...');
  ...
  const totalCost = successful * 0.08;
