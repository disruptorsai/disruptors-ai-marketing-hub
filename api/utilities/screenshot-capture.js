  ...
  try {
    ...
    if (!pageRoute || !changeDescription) {
      return response.status(400).json({
        error: 'Missing required fields: pageRoute, changeDescription'
      });
    }
    ...
    if (!VIEWPORTS[viewport]) {
      return response.status(400).json({
        error: `Invalid viewport: ${viewport}`
      });
    }
