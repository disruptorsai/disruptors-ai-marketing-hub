...
import { z } from 'zod';
import { OPPORTUNITY_DETECTOR_SYSTEM, buildOpportunityPrompt } from './prompts.js';
...
/**
 * Detect growth opportunities using Claude Sonnet 4.5
 * @param {import('../types.js').BusinessProfile} profile - Business profile to analyze
 * @param {number} [readinessScore=50] - Initial readiness score
 * @returns {Promise<{opportunities: import('../types.js').QuickWin[], readinessScore: number, assumptions?: string[]}>} Detected opportunities with readiness score
 */
export async function detectOpportunities(profile, readinessScore = 50) {
...
/**
 * Calculate readiness score based on business profile completeness
...
/**
 * Prioritize opportunities by impact/effort ratio and confidence
 * @param {import('../types.js').QuickWin[]} opportunities - Array of opportunities
 * @returns {import('../types.js').QuickWin[]} Sorted opportunities (highest priority first)
 */
